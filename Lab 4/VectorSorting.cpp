...
int partition(vector<Bid>& bids, int begin, int end) {
	//local variables for the partition
	int lowest = begin, highest = end;
	...
	while (!finished) {
		...
		while (bids.at(lowest).title < pivot) {
			++lowest;
		}
		//Deicnrement high while title > pivot
		while (pivot < bids.at(highest).title) {
			--highest;
		}
		//Completion check
		if (lowest >= highest)
			...
			//Swap bids
			swap(bids.at(lowest), bids.at(highest));
			++lowest;
			--highest;
		}
	}
...
void selectionSort(vector<Bid>& bids) {
	...
	Bid tempBid = bids.at(i);
	bids.at(i) = bids.at(smallest);
	bids.at(smallest) = tempBid;*/
